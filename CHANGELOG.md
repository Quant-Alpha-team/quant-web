## v0.6.7 (2026-07-24)

### Fix

- **dashboard**: add context to trade KPI cards
- **dashboard**: improve resizable table column behavior

## v0.6.6 (2026-07-22)

### Fix

- **dashboard**: show zero for missing realized P&L values

## v0.6.5 (2026-07-20)

### Fix

- **dashboard**: use realized P&L for realized KPI

## v0.6.4 (2026-07-20)

### Fix

- **dashboard**: remove comma from formatted timestamps

## v0.6.3 (2026-07-19)

### Fix

- **dashboard**: improve table sorting and responsive layout

## v0.6.2 (2026-07-18)

### Fix

- **dashboard**: sync holdings expand and collapse control state

## v0.6.1 (2026-07-18)

### Fix

- recover build image problem

## v0.6.0 (2026-07-18)

### Feat

- **dashboard**: add strategy-aware portfolio, positions, and P&L views

### Refactor

- **dashboard**: remove unused dashboard code

## v0.5.3 (2026-07-04)

### Refactor

- update gitlab's registry link

## v0.5.2 (2026-06-22)

### Fix

- **log**: set default LOG_PATH as logs

## v0.5.1 (2026-06-21)

### Fix

- **web**: replace quantX to Quant Aplha

## v0.5.0 (2026-06-19)

### Feat

- **web**: update new logo and icon

## v0.4.4 (2026-06-19)

### Fix

- **sync**: recover the wrong equity while using ALL Equity part
- **web**: fix the Refresh does not work about new data

## v0.4.3 (2026-06-13)

### Fix

- **web**: make dashboard clear

## v0.4.2 (2026-06-13)

### Fix

- **web**: fix the wrong Dockerfile settings

## v0.4.1 (2026-06-13)

### Fix

- **web**: fix the wrong semver version
- **web**: update build.sh

## v0.4.0 (2026-06-13)

### Feat

- **web**: rename Quant X as Quant Alpha

## v0.3.1 (2026-05-09)

### Fix

- **build**: enforce clean branch clone and valid lock versions

## v0.3.0 (2026-05-09)

### Feat

- align trading API routes and add month-to-date preset

## v0.2.6 (2026-05-01)

### Fix

- **log**: replace dashboard-YYYY-MM-DD.log to dashboard.log.YYYY-MM-DD

## v0.2.5 (2026-04-24)

### Fix

- **web**: fix the ubuntu's ui problem

## v0.2.4 (2026-04-24)

### Fix

- **docker**: remove useless public part

## v0.2.3 (2026-04-24)

### Feat

- **log**: add log's file part

## v0.2.2 (2026-04-25)

### Feat

- **docker**: add docker's part

### Fix

- **env**: remove env's yaml

## v0.2.1 (2026-04-25)

### Fix

- add missing changelog

## v0.2.0 (2026-04-25)

### Feat

- **web**: add the first version for quant-web

## v0.1.0 (2026-04-24)

### Init

- **web**: add the first version for quant-web
